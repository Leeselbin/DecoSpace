...
const Furniture: React.FC<FurnitureProps> = ({
  ...
  const [shape, setShape] = useState(item.shape);
  ...
  const changeColor = () => {
    color === 'lightblue' ? setColor('lightgreen') : setColor('lightblue');
    toggleOptions();
  };
  ...
  useEffect(() => {
    if (gestureMode === 'saving') {
      console.log(itemIndex, shape, color);
    }

    if (gestureMode === 'reset') {
      setShape(furnitureList[itemIndex].shape);
      setColor(furnitureList[itemIndex].color);

      startX.value = 0;
      startY.value = 0;
      translateX.value = furnitureList[itemIndex].x;
      translateY.value = furnitureList[itemIndex].y;
    }
  }, [gestureMode]);
  ...
  return (
    <GestureHandlerRootView>
      <GestureDetector
        gesture={gestureMode === 'edit' ? composedGesture : Gesture.Race()}>
        {getShapeComponent()}
      </GestureDetector>
